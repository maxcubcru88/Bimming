...
KEYWORD             = "SKIP"
PATH_EXCEL          = r"C:\Users\34644\AppData\Roaming\Github pyRevit\Bimming.extension\Bimming.tab\Modify.Panel\Type Parameters.pushbutton\000-WINDOWS-Window Types.xlsx"
...
# Project Number
r'Z:\511_Horatia House and Leamington House\03 Technical\01 BIM\01 Revit\10 Resources\Type Parameters Tables'
...
normalized_name = re.sub(r'[^A-Z]', '', dynName.upper())
...
for elem_type in collector:
    ...
    for parameter in parameters:
        ...
        if revit_parameter:
            ...
            parameter_parameter_type = revit_parameter.Definition.ParameterType
            parameter_value_from_excel = data_dict[model_value][parameter]
            if parameter_storage_type == StorageType.Integer:
                if parameter_parameter_type == ParameterType.YesNo:
                    if parameter_value_from_excel.upper() == 'TRUE':
                        parameter_value = 1
                    elif parameter_value_from_excel.upper() == 'FALSE':
                        parameter_value = 0
                    else:
                        storage_type = "YES/NO"
                        error = True
                ...
                if parameter_parameter_type == ParameterType.Length:
                    try:
                        ...
                        storage_type = "DOUBLE"
                        error = True
                elif parameter_parameter_type == ParameterType.Area:
                    try:
                        parameter_value_ = float(parameter_value_from_excel)
                        ...
                        storage_type = "DOUBLE"
                        error = True
                elif parameter_parameter_type == ParameterType.Number:
                    try:
            ...
            if error:
                if parameter_parameter_type == ParameterType.YesNo:
                    error_description_2 = ("The parameter '{}' is an {} (Parameter Type = {}) and '{}' could not be converted. Use TRUE or FALSE."
                                        .format(parameter, storage_type, parameter_parameter_type, str(parameter_value_from_excel)))
                    errors_2.append([model_value, parameter, error_description_2])
                    continue
                else:
                    error_description_2 = ("The parameter '{}' is an {} (Parameter Type = {}) and '{}' could not be converted."
                                        .format(parameter, storage_type, parameter_parameter_type, str(parameter_value_from_excel)))
                    errors_2.append([model_value, parameter, error_description_2])
                    continue
